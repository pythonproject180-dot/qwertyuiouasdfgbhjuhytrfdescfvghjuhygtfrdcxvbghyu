...
def register_password(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            name_parts = reg_data['full_name'].strip().split(' ', 1)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if account and account.check_password(password):
                ...
                return redirect('patient_portal:dashboard')
            messages.error(request, 'Invalid Hospital ID, phone number, or password.')
    else:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            write_audit_log(
                ...
                f"Self-service visit booked via Patient Portal: {patient.full_name}",
                patient_id_text=patient.patient_code, receipt_number=visit.receipt_number,
                amount=visit.registration_fee,
            )
            messages.success(request, f'Visit booked. Receipt: {visit.receipt_number}, Token: {visit.token_number}')
            return redirect('patient_portal:my_ticket', visit_id=visit.id)
    else:
        form = PatientVisitBookingForm(initial={'patient_type': Visit.PatientType.OLD})
