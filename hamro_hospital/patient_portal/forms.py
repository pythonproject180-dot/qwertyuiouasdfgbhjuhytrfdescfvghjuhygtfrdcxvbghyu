...
class PatientSignupForm(forms.Form):
    ...
    phone_number = forms.CharField(
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Phone number on your patient record'}),
    )
    ...
    def clean(self):
        ...
        if hospital_id and phone_number:
            try:
                patient = Patient.objects.get(patient_code__iexact=hospital_id, phone_number=phone_number)
            except Patient.DoesNotExist:
                raise forms.ValidationError(
                    'We could not find a patient record matching that Hospital ID and phone number. '
...
class PatientVisitBookingForm(forms.ModelForm):
    ...
    class Meta:
        model = Visit
        fields = ['department', 'doctor', 'patient_type', 'chief_complaint']
        widgets = {
