...
class PatientForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # below, so it isn't required from the user directly - they can still
        # override it manually if the approximate calculation is off.
    ...
    def clean(self):
        ...
        if not has_insurance:
            # Clear insurance fields if the box isn't checked, per spec.
            for field in ['insurance_company', 'insurance_policy_number', 'insurance_membership_number',
                ...
                cleaned_data[field] = None if field == 'insurance_company' or field == 'insurance_expiry_date' else ''

        # Age entry -> Date of Birth (spec section 12): if the user didn't
        # manually set/override a Date of Birth, calculate it from the
        # compulsory age_input text; if they typed both, the manual DOB wins.
