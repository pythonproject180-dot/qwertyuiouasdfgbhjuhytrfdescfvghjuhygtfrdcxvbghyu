...
def patient_register(request):
    """Register a brand-new patient and immediately create their first OPD visit."""
    if request.method == 'POST':
        ...
        if patient_form.is_valid() and visit_form.is_valid():
            ...
            if candidate_visit.doctor and not is_super_admin:
                ...
                quota_error_context = {'patient_form': patient_form, 'visit_form': visit_form, 'new_fee': 100, 'old_fee': 50}
            ...
            write_audit_log(
                ...
                f"Registered new patient {patient.full_name}",
            ...
            if visit.payment_status == Visit.PaymentStatus.PENDING:
                create_notification(
                    ...
                    message=f"Walk-in registration fee pending for {patient.full_name}. Amount: NPR {visit.registration_fee}.",
                    role=Role.CASH_COUNTER
                )
            messages.success(request, f'Patient registered successfully. Patient ID: {patient.patient_code}')
            return redirect('patients:opd_ticket', visit_id=visit.id)
    else:
        patient_form = PatientForm()
        visit_form = VisitForm(initial={'patient_type': Visit.PatientType.NEW})
...
def qr_lookup(request):
    """
    JSON lookup used by the QR/barcode scanner widget on every module's
    Search Patient screen. Accepts the value encoded in the Patient's QR
    code (their patient_code) or a phone number, returns the fields needed
    to auto-fill a form: Patient ID, Name, Phone, QR code image URL.
    """
...
def patient_new_visit(request, pk):
    ...
    if request.method == 'POST':
        ...
        visit_form = VisitForm(initial={'patient_type': Visit.PatientType.OLD})
